...
def fuzzy_contains(kw, text, threshold=75):
    ...
    if not words:
        return False
    # check single-word fuzzy matches
    for w in words:
...
GLOBAL_EDU = {"bachelor","master","mba","b tech","b.tech","m.tech","phd","degree","bsc","msc"}
...
def score_resume_against_jd(resume_text, jd_text, jd_keywords, fuzzy_threshold=78):
    ...
    if yrs:
        ...
        exp_pct = min(100, found * 40)
    breakdown["experience"] = round(exp_pct, 2)

    # fallbacks if JD lacks explicit words
    general_jd = jd_keywords.get("general", [])
    if not categories["skills"]:
        gslice = general_jd[:10]
        gmatch = sum(1 for kw in gslice if fuzzy_contains(kw, res_norm, threshold=fuzzy_threshold))
        breakdown["skills"] = round((gmatch / max(1, len(gslice))) * 100, 2)
        matched["skills"] = [kw for kw in gslice if fuzzy_contains(kw, res_norm, threshold=fuzzy_threshold)]
        missing["skills"] = [kw for kw in gslice if not fuzzy_contains(kw, res_norm, threshold=fuzzy_threshold)]
    if not categories["tools"]:
        gtools = [t for t in GLOBAL_TOOLS if t in jd_norm]
        if gtools:
            ...
            gslice2 = general_jd[10:20]
            gmatch = sum(1 for kw in gslice2 if fuzzy_contains(kw, res_norm, threshold=fuzzy_threshold))
            breakdown["tools"] = round((gmatch / max(1, len(gslice2))) * 100, 2)
            matched["tools"] = [kw for kw in gslice2 if fuzzy_contains(kw, res_norm, threshold=fuzzy_threshold)]
            missing["tools"] = [kw for kw in gslice2 if not fuzzy_contains(kw, res_norm, threshold=fuzzy_threshold)]
...
def generate_recommendations(jd_name, jd_keywords, score_details):
    """Generate personalized recommendations to improve ATS fit."""
    recs = []
    breakdown = score_details.get("breakdown", {})
    missing = score_details.get("missing", {})
    
    # Category-based improvement tips
    for cat, pct in breakdown.items():
        if pct is None:
            continue
        if pct < 70:
            if cat == "skills":
                recs.append(f"Add more domain-specific skills related to {jd_name}, such as {', '.join(jd_keywords.get('skills', [])[:5])}.")
            elif cat == "tools":
                recs.append(f"Include practical experience or certification in tools like {', '.join(jd_keywords.get('tools', [])[:5])}.")
            elif cat == "education":
                recs.append("Consider highlighting your academic background or relevant coursework that aligns with this JD.")
            elif cat == "experience":
                recs.append("Add measurable achievements or project details showing your experience in similar roles.")
    
    # Missing keywords
    for cat, miss in missing.items():
        if miss:
            addable = ", ".join(miss[:5])
            recs.append(f"Add or emphasize keywords like: {addable}.")
    
    # Tailor recommendations by JD role
    jdlower = jd_name.lower()
    if "data" in jdlower:
        recs.append("Showcase data-driven projects, SQL queries, or dashboards you built (Tableau/Power BI).")
    elif "marketing" in jdlower:
        recs.append("Add campaign results, SEO metrics, or analytics tools experience (Google Analytics, HubSpot).")
    elif "manager" in jdlower:
        recs.append("Highlight leadership, team management, and cross-functional collaboration examples.")
    elif "analyst" in jdlower:
        recs.append("Quantify insights or recommendations you produced and the business impact achieved.")
    elif "developer" in jdlower or "engineer" in jdlower:
        recs.append("List specific technical stacks, frameworks, or code repositories to showcase your skills.")
    
    if not recs:
        recs.append("Your resume already aligns well with this JD. You may fine-tune wording to improve ATS keyword detection.")
...
# Show plain DataFrame
st.dataframe(df, width='stretch')

# Color-coded heatmap using Plotly
try:
    matrix_values = df[jd_columns].fillna(0).to_numpy()
    fig_heat = px.imshow(matrix_values,
                        labels=dict(x="JD", y="Candidate", color="Score"),
                        x=[col.replace(" Overall", "") for col in jd_columns],
                        y=df["Candidate"].tolist(),
                        color_continuous_scale="RdYlGn",
                        aspect="auto")
    fig_heat.update_layout(height=600, width=None)
    st.plotly_chart(fig_heat, width='stretch')
except Exception as e:
    st.warning("Heatmap visualization failed: " + str(e))
...
for jd in jd_list:
    ...
    leaderboard = df[["Candidate", colname]].sort_values(by=colname, ascending=False).head(20).copy()
...
if sel_mode == "Select JD":
    ...
    fig_bar = px.bar(vis_df, x="Candidate", y=colname, title=f"Top candidates for {jd_choice}")
...
recs = generate_recommendations(best_jd, jd_keywords_map[best_jd], details)
...
st.markdown("---")
st.info("Notes: Matching uses fuzzy similarity (rapidfuzz). If JD doesn't list specific skills/tools, the system falls back to JD top tokens to estimate relevance. Use CSV downloads for offline filtering/HR workflows.")
